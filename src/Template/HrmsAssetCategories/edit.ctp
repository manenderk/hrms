 ...
 * @var \App\Model\Entity\HrmsAssetCategory $hrmsAssetCategory
 */
?>
<div class="row">
    <div class="col-md-12">
        <div class="page-title">ASSET CATEGORIES <small>Edit Category</small>
...
<?= $this->Form->create($hrmsAssetCategory, ['id'=>'departments']); ?>
<div class="wrapper bg-white atsborder">
    <!-- START panel-->
    <div class="panel-body">
        <div class="row form-group">
            <div class="col-lg-12 col-sm-12 col-md-9 col-xs-12">
                <label class="control-label text-center">Category Name</label>
                <?php echo $this->Form->control('category_name', ['label'=>false, 'placeholder'=>'Enter category name','class'=>'bg-gray form-control']); ?>
        ...
        <div class="row form-group">
            <div class="col-lg-12 col-sm-12 col-md-9 col-xs-12">
                <label class="control-label text-center">Category Name</label>
                <?php echo $this->Form->control('category_description', ['label'=>false, 'type' => 'textarea', 'placeholder'=>'Enter category description','class'=>'bg-gray form-control']); ?>
                <span id='message'></span>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        $("#category-name").blur(function() {
            ...
            $('#message').show();
            $('#message').html(
                "<img src='/img/loading.gif'>&nbsp;&nbsp;<span style='font-weight: bold;font-size:12px;'>Checking Category....</span>"
            );
            $.ajax({
                type: "POST",
                url: "<?=$this->Url->build(["controller" => "hrms-asset-categories","action" => "check-category-name", $hrmsAssetCategory->id]);?>",
                data: dataString,
                cache: false,
                headers: {
                ...
                success: function(result) {
                    ...
                    if (obj.success == 'true') {
                        $('#message').show();
                        $('#message').html('Category already exist! Please try another');
